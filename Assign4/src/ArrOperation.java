	...
	if( arr.length != 0 ){
		...
		/*if we find the same element then we will calculate the length of mirror formed*/
		    if( arr[i] == arr[j] ){
			    ...
			    if( arr[k] == arr[l] ){
			        count++;
				if(k == l){    //If the mirror is contigious the length will be calculated as follows
				count = count*2-1;
				}
			     }
 ...
 */
    public static int[] fixXY(int arr[],int x,int y){
	    ...
	    boolean flag = true; //true if array has no adjacent X
	    ...
	    // check if array has two adjacent X
	  ...
	  /*swapping position of y in array*/
		...
		return arr;
	        ...
	        throw new AssertionError("array contains unequal number of X and Y");
	    }
	}
	else{
	    throw new AssertionError("Array is empty");
